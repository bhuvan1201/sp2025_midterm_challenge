...
################################################################################
# Model Definition - Using ResNet101 for Transfer Learning
################################################################################
class ResNet101FineTune(nn.Module):
    def __init__(self):
        super(ResNet101FineTune, self).__init__()
        self.model = models.resnet101(pretrained=True)  # Load pretrained ResNet101
...
################################################################################
# Training Function with Mixup and Gradient Clipping
################################################################################
...
def main():
    ...
    model = ResNet101FineTune().to(CONFIG["device"])
    wandb.watch(model)
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer, max_lr=CONFIG["learning_rate"], steps_per_epoch=len(trainloader), epochs=CONFIG["epochs"]
    )
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if val_acc > best_val_acc:
            ...
            wandb.save("best_model_resnet101.pth")
    ...
    submission_df_ood.to_csv("submission_ood3_resnet101.csv", index=False)
    print("submission_ood3_resnet101.csv created successfully.")
